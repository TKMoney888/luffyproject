      ...
      <div class="condition">
        ...
        <div class="ordering">
          <ul>
            ...
            <li class="default" :class="orders=='id'?'this asc':(orders=='-id'?'this desc':'')" @click="select_course_ordering('id')">默认</li>
            <li class="hot" :class="orders=='students'?'this asc':(orders=='-students'?'this desc':'')" @click="select_course_ordering('students')">人气</li>
            <li class="price" :class="orders=='price'?'this asc':(orders=='-price'?'this desc':'')" @click="select_course_ordering('price')">价格</li>
          </ul>
      ...
      <div class="course-list">
        <div class="course-item" v-for="course in course_list">
          ...
          <div class="course-info">
            ...
            <p class="teather-info">{{course.teacher.name}} {{course.teacher.signature}} {{course.teacher.title}} <span>共{{course.lessons}}课时/{{course.lessons==course.pub_lessons?'已更新完成':`已更新${course.pub_lessons}课时`}}</span></p>
            <ul class="lesson-list">
              <li v-for="lesson,key in course.recomment_lesson_list"><span class="lesson-title">0{{key+1}} | 第{{lesson.number}}节：{{lesson.name}}</span> <span class="free" v-if="lesson.free_trail">免费</span></li>
            </ul>
            <div class="pay-box" v-if="course.discount_name">
              ...
              <span class="original-price" v-if="course.discount_price != course.min_price">原价：{{course.min_price.toFixed(2)}}元</span>
  ...
  export default {
      ...
      data(){
        return{
          ...
          course_list:[],   // 课程列表
          orders: "id",     // 课程排序字段说明,-id按id转尽心倒序排列
          course_total: 0,  // 课程总数
          page: 1,          // 当前页页码
          size: 5,          // 每一页显示的数据量
        }
